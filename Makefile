...
.PHONY: bootstrap pull build restart fleet-setup fleet-enable fleet-disable mysql-tune collect collect-backups instrument-on instrument-off pprof-view netdata-view finish publish bench help
...
restart: ## 全サーバーを OS 再起動する（追試・永続化確認用。initialize は走らない）
	@$(PLAYBOOK) tools/isucon-bench/ansible/restart.yml

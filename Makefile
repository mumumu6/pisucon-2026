...
.PHONY: help bootstrap server-sync pull deploy restart fleet-setup fleet-enable fleet-disable \
	mysql-tune collect collect-backups pprof-view netdata-view finish publish bench
...
deploy: ## server-sync のあと systemd設定・ビルド・app/nginx 再起動まで行う
...
fleet-setup: ## 計測ツール導入＋計測系（netdata/slow query/pprof）を有効にする
	@$(PPROF_SCRIPT) on
	@$(PLAYBOOK) $(ANSIBLE_DIR)/setup.yml
	@$(MAKE) --no-print-directory server-sync
	@$(PLAYBOOK) --extra-vars monitor_state=on $(ANSIBLE_DIR)/monitor.yml
...
fleet-enable: ## 計測系（netdata / slow query / pprof）を有効にする
	@$(PPROF_SCRIPT) on
	@$(MAKE) --no-print-directory server-sync
	@$(PLAYBOOK) --extra-vars monitor_state=on $(ANSIBLE_DIR)/monitor.yml
	@$(MAKE) --no-print-directory collect-backups

fleet-disable: ## 計測系（netdata / slow query / pprof）を止める
	@$(PPROF_SCRIPT) off
	@$(MAKE) --no-print-directory server-sync
...
finish: ## 最終計測前に計測系（netdata / slow query / pprof）を外す
